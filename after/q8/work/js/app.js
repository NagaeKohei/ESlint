$(function () {
  let pageCount = 1, searchWord = ''; // ページ番号とキーワードを初期化

  $('.search-btn').on('click', function () {
    const input = $('#search-input').val(); // 入力値を取得
  ...
  $('.reset-btn').on('click', function () {
    ...
    function displayResult(books) {
      $('.message').remove(); // メッセージを削除
      const length = books[0].items?.length; // オプショナルチェイニングでitemsが存在するかチェック
      if (length > 0) { // 0より大きいかどうか比較
        $.each(books[0].items, function (index, book) {
          const item = `<li class="lists-item"><div class="list-inner"><p>タイトル：${book.title ?? 'タイトル不明'}</p><p>作者：${book['dc:creator'] ?? '作者不明'}</p><p>出版社：${book['dc:publisher']?.[0] ?? '出版社不明'}</p><a href='${book.link['@id']}' target='_blan'>書籍情報</a></div></li>`; // テンプレートリテラルで文字列を作成
